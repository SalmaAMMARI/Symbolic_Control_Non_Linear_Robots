...
def visualize_pybullet(robot, traj, actions, spec_ok, R1_bounds, R2_bounds, R3_bounds, R4_bounds):
    ...
    # Plane (small plane centered)
    p.loadURDF("plane.urdf", [robot.size/2, robot.size/2, 0], globalScaling=1.0)
    ...
    # Optional: draw faint grid tiles
    for idx, ((x0, x1), (y0, y1), _) in robot.index_to_intervals.items():
        cx, cy = (x0 + x1) / 2, (y0 + y1) / 2
        sx, sy = (x1 - x0) / 2, (y1 - y0) / 2
        vis = p.createVisualShape(p.GEOM_BOX, halfExtents=[sx, sy, 0.01], rgbaColor=[0.8, 0.8, 0.8, 0.03])
        p.createMultiBody(baseMass=0, baseVisualShapeIndex=vis, basePosition=[cx, cy, 0.01])

    # Trajectory rendering (polyline + small spheres)
    traj_color = [0, 0.7, 0] if spec_ok else [1.0, 0.6, 0.0]
    for i, pt in enumerate(traj):
        if i > 0:
            p.addUserDebugLine(
                [traj[i-1][0], traj[i-1][1], 0.12],
                [pt[0], pt[1], 0.12],
                traj_color,
                lineWidth=3
            )
        sphere_vis = p.createVisualShape(p.GEOM_SPHERE, radius=0.08, rgbaColor=traj_color + [0.9])
        p.createMultiBody(baseMass=0, baseVisualShapeIndex=sphere_vis, basePosition=[pt[0], pt[1], 0.12])

    # Robot model (use a simple URDF available in pybullet_data)
    start_pos = [traj[0][0], traj[0][1], 0.25]
    start_ori = p.getQuaternionFromEuler([0, 0, traj[0][2]])
    try:
        robot_id = p.loadURDF("r2d2.urdf", start_pos, start_ori)
    except Exception:
        # fallback to a sphere if URDF not available
        ...
        robot_id = p.createMultiBody(baseMass=1.0, baseCollisionShapeIndex=sphere_col, baseVisualShapeIndex=sphere_vis, basePosition=start_pos)
    ...
    print(" Simulation finished. Press Enter to close PyBullet window.")
    try:
        input()
    except Exception:
        pass
...
def main_dfa_pybullet():
    ...
    ctrl.compute_R_star(max_iters=120)
    ctrl.compute_controller()

    # Start bottom-left corner, heading NE
    initial_state = [0.5, 0.5, np.pi/4]
    traj, actions, pairs, ok = generate_controller_trajectory(robot, aug, ctrl, initial_state, steps=300)

    print(f"Trajectory length: {len(traj)}, specification satisfied: {ok}")
